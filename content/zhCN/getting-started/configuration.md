---
title: 配置
description: 您可以在本页中找到 GadgetsMenu 的所有配置设置。
...
### 我该如何禁用某一件化妆品？
您可以在 `/GadgetsMenu/categories` 文件夹中对应的文件里禁用该化妆品。

在下面的示例中，将使用 `hats.yml` 文件来演示如何禁用某一件化妆品。 

只需将 `Enabled` 属性设置为 `false` 即可禁用该化妆品。玩家将无法再装备这件化妆品，也无法从神秘箱中获得它。
```yaml
# Don't change this!
Hamburger:
  ...
...
### 我该如何禁止某件化妆品从神秘箱中获得？
您可以在 `/GadgetsMenu/categories` 文件夹中对应的文件里，禁止某件化妆品从神秘箱中获得。

在下面的示例中，将使用 `hats.yml` 文件来演示如何禁止某件化妆品从神秘箱中获得。 

只需将 `CanBeFound` 属性设置为 `false`，即可禁止该化妆品从神秘箱中获得。
```yaml
# Don't change this!
Hamburger:
  ...
...
### 我该如何在物品显示名称中配置十六进制颜色？
在 Minecraft 1.16 及更高版本中，您可以为显示名称和全息图使用十六进制颜色。
 - **语法：`{#<hex color>}`**
...
### GadgetsMenu 中支持配置哪些音效？

您可以在[此处](../wiki/others/sounds)找到所有音效名称的列表。
```
Play-Sound:
  Enabled: true
  Sound: ENTITY_ENDERMAN_TELEPORT
